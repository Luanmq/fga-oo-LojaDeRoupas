...
public class TelaMinhaSacola implements ListSelectionListener{
	...
	public TelaMinhaSacola() {
		int k = 0;
	    int l = 0;
	    
	    //Coloca todos os acessorios na sacola
    	for(int i = 0; i < Dados.usuarioPrincipal.getSacola().getSacolaParaAcessorios().size(); i++){
    		listaNomesAcessoriosSacola[k] =  Dados.usuarioPrincipal.getSacola().getSacolaParaAcessorios().get(i).getNome();
    		k++;
    	}
    	
    	//Coloca todos as roupas na sacola
    	for(int i = 0; i < Dados.usuarioPrincipal.getSacola().getSacolaParaRoupas().size(); i++){
    		listaNomesRoupasSacola[l] =  Dados.usuarioPrincipal.getSacola().getSacolaParaRoupas().get(i).getNome();
    		l++;
    	}
